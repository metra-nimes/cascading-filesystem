...
class CascadingFilesystem
{
	...
	/**
	 * @param $cache \Kohana_Cache object
	 * @param $base_paths Paths to directories, latter paths have precedence
	 */
	public function __construct(\Kohana_Cache $cache, array $base_paths)
	{
		$this->cache = $cache;
	...
	public function getPath($relative_path)
	{
		...
		if (($cached_data = $this->cache->get($cache_key)) !== false) {
			return $cached_data;
		}
		...
		foreach (array_reverse($this->base_paths) as $base_path) {
			...
			if (is_file($absolute_path)) {
				// Cache the file path
				$this->cache->set($cache_key, $absolute_path);
	...
	public function getAllPaths($relative_path)
	{
		...
		// Cache the result
		$this->cache->set($cache_key, $found);
